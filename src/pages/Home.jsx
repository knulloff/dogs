...
import { useFindAccountQuery, useLeaderboardQuery, usePointTableQuery, useTaskListQuery, useUpdateAccountMutation } from "../rtk/api/Endpoint";
import { TonConnectButton } from "@tonconnect/ui-react";
import bull_image from "../assets/images/bull-attacking.jpg";
const Home = () => {
    const { data, isFetching } = useTaskListQuery(localStorage.getItem("id"));
    const { data: PointData } = usePointTableQuery(localStorage.getItem("_id"));
    ...
    return (
        <div className="bg-black relative">
            <div className="p-3 ">
                ...
                <p className="text-3xl flex items-center justify-center gap-1 text-white font-medium text-center">{PointData?.point ? PointData?.point : 0}
                    <svg className={`${PointData?.userId?.transaction ? '' : 'hidden'}`} xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 16 16" fill="none">
                ...
                <p className="text-xl uppercase text-white text-center">Bulls</p>
            </div>

            {/* <Task_Details data={data}/> */}
            <div className="relative bg-white bg-opacity-10 px-3 min-h-[50vh]  border-white border-t backdrop-blur-2xl rounded-t-3xl">
                <TaskList data={data?.data?.result} isFetching={isFetching} />
            </div>
        </div>
    );
};
